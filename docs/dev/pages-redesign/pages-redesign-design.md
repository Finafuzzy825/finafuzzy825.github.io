...
按 requirements §2 与原型 `cybersecurity.html`（**最新中文文案直接来源**）重写。口径铁律：**厂商中立**、**仅点名已授权牵头方**（智谱 / 清华网络与信息安全实验室 / 数说安全 / 云起无垠）、受邀企业只用角色描述、**不放具体申请数字 / 个人姓名 / GLM 独家或竞争先发口径**。
...
- **新增** `organisation`（6 卡，点名已授权牵头方：联盟统筹 · 智谱模型技术牵引「对不同厂商对等开放」· 清华网络与信息安全实验室学术指导 · 数说安全 & 云起无垠联合运营 · 生态伙伴共建 · 监管协同）。
